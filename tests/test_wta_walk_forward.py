"""Walk-forward harness sanity tests for the WTA feature-expansion harness.

A PORT of ``tests/test_atp_walk_forward.py``. Mirrors it in structure: small
synthetic datasets, no network, no I/O. The one catalog difference is that
WTA is best-of-3 only, so the ATP ``best_of_5`` feature is dropped — the
feature vector is length 13, and there are no best_of_5 assertions.
...
Verifies:
  ...
  * Feature vector length matches FEATURE_NAMES (13) and None-fill works.
  * best_of_5 is NOT in the WTA catalog.
...
from flashcat.wta_features.feature_builder import (
    FEATURE_NAMES,
    MatchRow,
    RollingWTAFeatures,
    _canonical_pair,
...
from flashcat.wta_features.model import (
    WalkForwardSplit,
    make_splits,
    walk_forward_evaluate,
)
from flashcat.wta_features.simulator import (
...
def _m(d, p1, p2, p1_won, *, surface="Hard", best_of=3, hrank=50, arank=60,
       ...
       market=None, bt=None, hdec=1.8, adec=2.0, tour="wta"):
...
def test_canonical_pair_alpha_first():
    assert _canonical_pair("Gauff C.", "Swiatek I.") == ("Gauff C.", "Swiatek I.", False)
    home, away, swap = _canonical_pair("Swiatek I.", "Gauff C.")
    assert home == "Gauff C." and away == "Swiatek I." and swap is True
...
def test_event_id_stable_regardless_of_order():
    a = event_id("wta", date(2023, 6, 1), "Swiatek I.", "Gauff C.")
    b = event_id("wta", date(2023, 6, 1), "Gauff C.", "Swiatek I.")
...
# ---------------------------------------------------------------------------
# 11-14. Feature gate + vector (WTA: 13 features, no best_of_5)
# ---------------------------------------------------------------------------

def test_feature_catalog_is_13_and_excludes_best_of_5():
    """WTA is best-of-3 only — best_of_5 must NOT be in the catalog."""
    assert len(FEATURE_NAMES) == 13
    assert "best_of_5" not in FEATURE_NAMES
...
def test_classify_loss_generic_fallback():
    ...
    feats = {"surface_win_pct_l20_diff": 0.0, "rank_points_log_ratio": 0.0,
             "rest_days_diff": 0.0, "h2h_home_share": 0.5}
    assert _classify_loss(bet, feats, {}) == "generic"


# ---------------------------------------------------------------------------
# 18-20. End-to-end (synthetic, no I/O)
# ---------------------------------------------------------------------------
