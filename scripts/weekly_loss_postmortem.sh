...
for p in paths:
    ...
    per_sport_meta[sport] = {
        ...
        # Fall back to top-level roi/clv_proxy_pp (NFL schema) when the older
        # MLB-style ``overall`` block isn't present.
        "roi": overall.get("roi") if overall.get("roi") is not None else data.get("roi"),
        "clv_proxy_pp": overall.get("clv_proxy_pp") if overall.get("clv_proxy_pp") is not None else data.get("clv_proxy_pp"),
