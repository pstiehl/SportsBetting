#!/usr/bin/env python3
"""Walk-forward WTA backtest driver — feature-expansion Phase 1 (Week 6).

A PORT of ``scripts/atp_walk_forward_backtest.py`` to the women's tour.

Loads 2022-2024 WTA main-tour singles matches from the cached tennis-data.co.uk
...
13-feature Phase-1 catalog (ATP's 14 minus ``best_of_5`` — WTA is best-of-3
only), runs sliding-window logistic regression, and simulates a $100 flat-stake
bet on every model-graded match using the REAL archived closing decimal odds
(Pinnacle preferred).

Outputs:
  * data/wta_walk_forward_backtest.json                 — machine-readable receipt
  * paw-reports/sportsbetting/wta-week6-pr-body.md      — PR body with tables
...
Usage:
    PYTHONPATH=src python3 scripts/wta_walk_forward_backtest.py
    PYTHONPATH=src python3 scripts/wta_walk_forward_backtest.py --start 2022-01-01 --end 2024-12-31
"""
...
from flashcat.wta_features import (  # noqa: E402
    FEATURE_NAMES,
    load_matches_from_cache,
    fit_rolling_rates,
    attach_priors_from_db,
)
from flashcat.wta_features.feature_builder import compute_h2h  # noqa: E402
from flashcat.wta_features.model import walk_forward_evaluate  # noqa: E402
from flashcat.wta_features.simulator import simulate_flat_stake  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--tour", default="wta")
    p.add_argument("--out", default=str(DATA_DIR / "wta_walk_forward_backtest.json"))
    p.add_argument("--pr-body", default=str(REPO_ROOT / "paw-reports" / "sportsbetting" / "wta-week6-pr-body.md"))
...
def _format_pr_body(out_summary: dict, gated_summary: dict, ungated_summary: dict,
    ...
    lines = [
        ...
        "",
        f"Week 6 of the weekly sport-rotation feature-expansion loop. A near-mechanical PORT of the Week-5 ATP harness to the WTA women's tour. Seasons backtested: {seasons[0]}-{seasons[-1]}.",
        "",
        "### Backtest window",
        "",
        f"* **Start:** {args.start}",
        ...
        "### Data",
        "",
        f"* **Matches loaded** (WTA main-tour singles, completed): {n_loaded}",
    ...
    if roi > 0.01:
        ...
        verdict = f"Loses to the closing line — {roi*100:+.2f}% ungated ROI (CLV proxy {clv*100:+.2f}pp); the 13-feature Phase-1 catalog does not beat Pinnacle's close."
    else:
...
def main() -> int:
    ...
    log = logging.getLogger("wta_walk_forward")
    args = parse_args()
    start = _date(args.start)
    end = _date(args.end)
    seasons = list(range(start.year, end.year + 1))

    log.info("loading WTA matches from cache for seasons %s ...", seasons)
    ...
    out_summary = {
        ...
        "skipped_sources": ["sackmann-wta-elo (GitHub 404/429)", "serve/return point stats (not published)", "line movement (no opener archived)"],
        "dropped_features": ["best_of_5 (WTA is best-of-3 only; constant 0.0, zero signal)"],
    ...
    print()
    print("=" * 78)
    print(f"WTA Walk-Forward Backtest — {args.start} → {args.end}")
    print("=" * 78)
