"""Walk-forward logistic regression for the expanded NFL feature set.

Same structure as ``mlb_features.model`` — L2 logistic, walk-forward
365d train / 30d eval / 30d slide, leakage gate asserted in-loop.
"""
...
def _train_one_split(train_examples):
...
def make_splits(
    ...
    *,
    ...
    """Sliding splits. First eval window starts ``warmup_days`` after ``start``."""
...
def walk_forward_evaluate(
    ...
    *,
    ...
    log.info(
        "walk-forward NFL: %d games -> %d with full features; %d splits over %s..%s",
    ...
    for split in splits:
        ...
        for g in games:
            if id(g) not in feats_by_game or g.home_won is None:
                continue
        ...
        preds: list[dict] = []
        for g in eval_games:
            assert split.eval_start <= g.game_date <= split.eval_end, (
            ...
            preds.append({
                ...
                "epa_prob_home": g.epa_prob_home,
                "market_prob_home": f.get("market_prob_home"),
