...
def _settlement_decimals(
    rs: list[dict],
) -> tuple[float | None, float | None, bool | None]:
    """Recover both sides' decimal odds for an event from its source rows.

    The ``predictions`` table stores ``market_close_decimal`` per source
    as "decimal odds taken on the picked side" (the side that source had
    higher probability for). A two-way market has two distinct decimals
    that only collapse to one number in the balanced -110/-110 case.

    To settle a *blended* pick correctly we must know the decimal for
    each side independently:

      * Rows whose source picked HOME (``home_prob >= 0.5``) carry the
        home-side decimal in ``market_close_decimal``.
      * Rows whose source picked AWAY (``home_prob < 0.5``) carry the
        away-side decimal.

    When only one side is observed in the rows, we derive the other from
    the devigged home probability ``market_close_home`` (``p_h``) via the
    standard no-vig two-way inverse:

        p_h = (1 / home_dec) / ((1 / home_dec) + (1 / away_dec))
        ⇒ away_dec = p_h * home_dec / (1 - p_h)
        ⇒ home_dec = (1 - p_h) * away_dec / p_h

    Returns ``(home_decimal, away_decimal, home_won)``. Either decimal
    may be ``None`` if neither side is observable AND ``market_close_home``
    isn't available for derivation; the caller must then skip the event.
    """
    ...
    p_h: float | None = None
    for r in rs:
        if home_won is None and r.get("home_won") is not None:
            home_won = bool(r["home_won"])
        if p_h is None and r.get("market_close_home") is not None:
            try:
                ph_val = float(r["market_close_home"])
                if 0.0 < ph_val < 1.0:
                    p_h = ph_val
            except (TypeError, ValueError):
                pass
        d_raw = r.get("market_close_decimal")
        if d_raw is None:
            continue
        try:
            d = float(d_raw)
        ...
        if d <= 1.0:
            continue
        try:
            src_home_prob = float(r["home_prob"])
        except (TypeError, ValueError, KeyError):
            continue
        if src_home_prob >= 0.5:
            if home_dec is None:
                home_dec = d
        else:
            if away_dec is None:
                away_dec = d
    # Derive the missing side from the devigged home prob when available.
    if home_dec is None and away_dec is not None and p_h is not None:
        # home_dec = (1 - p_h) * away_dec / p_h
        if p_h > 0.0:
            home_dec = (1.0 - p_h) * away_dec / p_h
    elif away_dec is None and home_dec is not None and p_h is not None:
        # away_dec = p_h * home_dec / (1 - p_h)
        if p_h < 1.0:
            away_dec = p_h * home_dec / (1.0 - p_h)
    return home_dec, away_dec, home_won
...
def _blended_roi(
    ...
    over present sources for that event), picks the higher-blended-prob
    side, settles at the **picked side's own** decimal odds (recovered via
    ``_settlement_decimals``).

    Two-way market correctness note: PR-19's original implementation
    grabbed ``market_close_decimal`` from the first row that had one,
    which inflated ROI whenever the first row's source had picked the
    opposite side from the blend (e.g. settling a blended HOME pick at
    an away-side 3.50 decimal stored by an away-leaning source). PR #21
    fixes the settlement to use the per-side decimal.

    Returns ``(roi, n_bets, n_wins)``.
    """
    ...
    for event_id, rs in by_event.items():
        ...
        if side_dec is None or side_dec <= 1.0:
            # No decimal recoverable for the side the blend wants to bet;
            # skip this event rather than settle at the wrong side's price.
            continue
        won = (pick_home and home_won) or (not pick_home and not home_won)
        n_bets += 1
        wagered += 100.0
        if won:
            profit += 100.0 * (side_dec - 1.0)
            wins += 1
        else:
            profit -= 100.0
    if wagered <= 0:
