"""Walk-forward feature builder for WTA matches — Phase 1.
...
This is a PORT of ``atp_features/feature_builder.py`` to the women's tour.
The one catalog difference: WTA is best-of-3 only, so the ATP ``best_of_5``
feature (constant 0.0 on the WTA) is DROPPED — 13 features instead of 14.
...
* ``tennis-rank-bt``— Bradley-Terry on WTA rank points. Read from the DB.
...
@dataclass
class MatchRow:
    """One completed WTA singles match, canonicalized (home = alpha-first)."""
    ...
    series: str   # International / Premier / WTA1000 / Grand Slam / ...
    round: str
    best_of: int
...
def _parse_match(row: dict, year: int, tour: str) -> Optional[MatchRow]:
    ...
    # WTA is best-of-3 only; default to 3 and keep whatever the sheet says.
...
def attach_priors_from_db(matches: list[MatchRow], db_path: Path) -> int:
    ...
    try:
        cur = conn.execute(
            ...
            "WHERE sport IN ('wta') AND source IN "
...
def _freeze(f: RollingWTAFeatures) -> RollingWTAFeatures:
    out = RollingWTAFeatures()
...
def fit_rolling_rates(
...
) -> dict[tuple[str, str], RollingWTAFeatures]:
    """Build a snapshot of every player's rolling form just BEFORE each match.

    Returns ``{(event_id, player_name): RollingWTAFeatures}``. The snapshot
    ...
    """
    state: dict[str, RollingWTAFeatures] = defaultdict(RollingWTAFeatures)
    snapshots: dict[tuple[str, str], RollingWTAFeatures] = {}
...
# Feature assembly
# ---------------------------------------------------------------------------
# NOTE (WTA-specific): the ATP catalog's ``best_of_5`` feature is DROPPED.
# The WTA main tour is best-of-3 only, so best_of_5 would be a constant 0.0
# across the whole dataset and adds zero signal. 13 features, not 14.
...
def build_features(
    match: MatchRow,
    rolling: dict[tuple[str, str], RollingWTAFeatures],
    h2h: dict[str, float],
) -> Optional[dict]:
    ...
    """
    home_f = rolling.get((match.event_id, match.home), RollingWTAFeatures())
    away_f = rolling.get((match.event_id, match.away), RollingWTAFeatures())
    ...
    def _rest(f: RollingWTAFeatures) -> Optional[float]:
