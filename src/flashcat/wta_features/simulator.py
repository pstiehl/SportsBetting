"""Flat-$100 simulator for the walk-forward WTA backtest.

A PORT of ``atp_features.simulator``. Phil's spec: place a hypothetical $100
wager on every model-graded match. No production edge gate on the headline
numbers (we ALSO emit the +3pp-gated subset for the report). Tennis-specific
loss buckets.

WTA-specific note: the ATP simulator has a ``best_of_5_variance`` bucket for
Grand Slam best-of-5 losses. The WTA main tour is best-of-3 only, so that
bucket can never fire here and is dropped (the ``best_of_5`` feature it keyed
off is not in the WTA catalog). All other tennis loss buckets port directly.
...
def _classify_loss(bet: dict, features: dict, ctx: dict) -> str:
    ...
    (The ATP ``best_of_5_variance`` bucket is intentionally absent — WTA is
    best-of-3 only.)
    """
    p = bet["pick_prob"]
    edge = bet["edge_pp"]
...
def simulate_flat_stake(
    ...
    *,
    ...
    """Simulate flat $100 bets on every model-graded WTA match.
    ...
    # Sharpe (per-bet, annualized to ~2600 WTA main-tour matches/season).
