"""ATP (men's tennis) feature-expansion harness — Week 5 of the weekly rotation.

Mirrors ``src/flashcat/nfl_features/`` in spirit: pure-function feature
builders, leakage-gated rolling rates, a sport-agnostic walk-forward
logistic harness (reused from ``nfl_features.model``), and a flat-$100
simulator with ATP-specific loss buckets.
...
Phase-1 feature catalog (14 features):
  Priors / market (3)
    market_prob_home        — devigged closing two-way moneyline (Pinnacle>B365>Avg)
    rank_bt_prob_home       — Bradley-Terry on ATP rank points (from source_history.db)
  ...
  Head-to-head + context (2)
    h2h_home_share          — home's historical H2H win share vs this opponent (0.5 prior)
    best_of_5               — 1.0 for Grand Slam best-of-5, else 0.0

Skipped in Phase 1 (documented in the PR):
  * sackmann-atp-elo surface-adjusted Elo — the JeffSackmann/tennis_atp
    GitHub repo returned 404/429 during the Week-5 backfill, so the Elo
    source never populated. Highest-value Phase-2 lever.
