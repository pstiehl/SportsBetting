"""Walk-forward logistic regression for the expanded ATP feature set.

Same structure and hyperparameters as ``nfl_features.model`` — L2 logistic
(C=1.0) with StandardScaler, walk-forward 365d train / 30d eval / 30d slide,
120d warmup, leakage gate asserted in-loop.
...
because the ATP feature builder takes ``(match, rolling, h2h)`` instead of
...
def walk_forward_evaluate(
    ...
    *,
    ...
    log.info(
        "walk-forward ATP: %d matches -> %d with full features; %d splits over %s..%s",
